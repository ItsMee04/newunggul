$(document).ready(function () {
    ...
    $(document).on("click", "#refreshButton", function () {
        if (suplierTable) {
            suplierTable.ajax.reload(); // Reload data dari server
        }
    ...
    function loadSuplier() {
        // Datatable
        if ($('.suplierTabel').length > 0) {
            suplierTable = $('.suplierTabel').DataTable({
                ...
                ajax: {
                    url: 'suplier/getSuplier', // Ganti dengan URL endpoint server Anda
    ...
    $(".btn-tambahSuplier").on("click", function () {
        $("#mdtambahSuplier").modal("show");
    });

    $("#storeSuplier").on("submit", function (event) {
        ...
        $.ajax({
            url: "/suplier", // Endpoint Laravel untuk menyimpan pegawai
            type: "POST",
            data: formData,
            ...
            success: function (response) {
                ...
                $("#storeSuplier")[0].reset(); // Reset form
                suplierTable.ajax.reload(); // Reload data dari server
            },
            error: function (xhr) {
    ...
    $(document).on("click", ".btn-edit", function () {
        ...
        $.ajax({
            url: `/suplier/${suplierID}`, // Endpoint untuk mendapatkan data pegawai
            type: "GET",
            success: function (response) {
                // Isi modal dengan data pegawai
                $("#editid").val(response.Data.id);
                $("#editkodesuplier").val(response.Data.kodesuplier);
                $("#editsuplier").val(response.Data.suplier);
    ...
    $(document).on("submit", "#formEditSuplier", function (e) {
        ...
        dataForm.append("kodesuplier", $("#editkodesuplier").val());
        dataForm.append("suplier", $("#editsuplier").val());
        ...
        $.ajax({
            url: `/suplier/${$("#editid").val()}`, // URL untuk mengupdate data pegawai
            ...
            success: function (response) {
                ...
                $("#formEditSuplier")[0].reset(); // Reset form
                suplierTable.ajax.reload(); // Reload data dari server
            },
            error: function (xhr) {
                const errors = xhr.responseJSON.errors;
